...
#the first thing was adding a new option at the beginning! So I have the -B paramete, for the band parameter options. Another thing: I put the options in alphabetic order
#not to be confused with the  -b paramter, which is from the batch
...
#@staticmethod
catalouge_name = "catalogue_MeerKAT_final.csv" #since i use this in two different places, it is good to have it here as a parameter
#maybe if we wanna be precise, we should have it as an option? Idk


def get_gauss_expr_string(params):
    """
    Gives back a string with a triple gaussian funciton, putting the input parameters as follows
    params: is the list of these params [norm1, mean1, sigma1, norm2, mean2, sigma2, norm3, mean3, sigma3]
    """
...
# Migrad remember that is a method that minimises the chi^2. So minuit just jnows how to minimise funcionts, and hence we need to define the hi squared to minimise
...
#            self.fluxexpr="10*0.5*0.5*1e-4*x**-2"
        ...
        if band==  "all":
         ...
         print("No bands used! Fit of the background without cuts")
          ...
          print(df.declination) #here i read the declination of the source directly from the catalogue. This allows me to compute the new mand extremes
          dec =  df.declination[int(candum)]
          s_dec = sin(dec)
          min ,max = s_dec-0.2, s_dec+0.2
          if min< -1: #these two are in the case we are too close to the limits of possible declination. 
#these lines allow to have a significant statistics,also for "edge" cases
            self.s_min = -1
            self.s_max = -0.8
          elif max>0.8:
            self.s_max = 0.8
         ...
         print("!!!!ATTENTION!!!!")
         print( "I did NOT recognise the band parameter you gave me. This means that I am going to go with a band wide as the whole declination band")
         print("!!!ATTENTION!!!")
         func = "1033.9462443148202*exp(-0.5*pow((x-3.593382403792754)/0.5532062653963777,2))+9.750099808580014e-07*exp(-0.5*pow((x-4.292137178498489)/3.3293596715027034,2))+316.17663638080796*exp(-0.5*pow((x-2.2435144672644527)/1.1909797511602955,2))" 
...
#copia arca 8 al posto di quella fuori. e basta così.             
        ...
        self.npe_h0       = 200000  # number of PEs to run
        self.npe_h1       = 50000
        self.npe_sigcheck = 1
